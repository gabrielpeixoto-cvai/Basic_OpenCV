...
public:
	...
	void createTemplate(){
		Mat input_img = imread("files/hand_original.jpg", 1);
		Mat src_gray;
		Mat detected_edges;
		...
		imwrite("files/hand_contour_rough.bmp", detected_edges);
	}
	...
	void bestCandidate(){
		...
		for(;;)
		{
			...
			if (alt){
				...
				imwrite( "result.jpg", input_img22);
			}
			else{
